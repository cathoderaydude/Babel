...
namespace Babel.Async.MicrosoftImpl
{
    ...
    public class AsyncTranslation : IAsyncTranslation
    {
        ...
        private async Task DoTranslation()
        {
            try
            {
                ...
                HttpRequestMessage request = new HttpRequestMessage();
                request.RequestUri = new Uri("https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to=" + Properties.Settings.Default.targetLocale);
                request.Method = HttpMethod.Post;
                request.Headers.Add(
                    "Ocp-Apim-Subscription-Key",
                    Properties.Settings.Default.microsoftTranslatorApiKey);
                ...
                _translatedText = json
                    .Select(x => (string)x["translations"][0]["text"])
                    .Aggregate((x, y) => x + Environment.NewLine + y);

                // Close enough
                _detectedLocale = (string)json.First()["detectedLanguage"]["language"];
